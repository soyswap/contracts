...
contract SoyVault is Ownable, ReentrancyGuard {
    ...
    // Deposit LP tokens to SoyVault for Soy allocation
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(
            block.number >= startBlock,
            "SoyVault: cannot deposit before farming start"
        );
    ...
    function safeSoyTransfer(address _to, uint256 _amount) internal {
        if (soy.balanceOf(address(this)) > totalSoyInPools) {
            //SoyBal = total Soy in SoyVault - total Soy in Soy pools, this will make sure that SoyVault never transfer rewards from deposited Soy pools
